<template>
    <div class="time-tracker">
        ...
        <VueDatePicker :locale="locale" v-model="date" range multi-calendars class="date-picker"></VueDatePicker>
        ...
        setup() {
            ...
            onMounted(() => {
                ...
                if (savedStartTime) {
                    ...
                    const now = Date.now();
                    duration.value = Math.floor((now - startTime) /
                    1000); // Calcular duración desde que se guardó el start_time
...
<style scoped>
    ...
    .main-row {
        display: flex;
        flex-direction: row;
        /* Mantenemos la fila para pantallas grandes */
        align-items: center;
        gap: 20px;
        margin-top: 20px;
        width: 100%;
