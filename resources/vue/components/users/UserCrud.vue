                ...
                <div class="modal-body">
                    <form @submit.prevent="updateReservation">
                        <div class="mb-3">
                            ...
                            <input id="client_name" type="text" class="form-control" v-model="reservation.client_name" required autocomplete="client_name" autofocus>
...
export default {
        ...
        updateReservation() {
            axios.put(`/reservations/${this.reservation.id}`, this.reservation)
                .then(response => {
                    ...
                    $('#reservations-table').DataTable().draw(); // Actualiza la tabla
                })
                .catch(error => {
    ...
    mounted() {
        // Escuchar eventos para cargar reserva
        $(document).ready(() => {
            $('body').on('click', '.btn-edit', (event) => {
                const reservationId = $(event.currentTarget).data('id');
